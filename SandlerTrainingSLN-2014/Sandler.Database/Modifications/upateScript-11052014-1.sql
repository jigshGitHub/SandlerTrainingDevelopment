IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_CompanyHasOpporunitiesView]') AND type in (N'P', N'PC'))
	DROP PROCEDURE [dbo].[sp_CompanyHasOpporunitiesView]
GO
/*
Similar kind of sp_CompanyView but this has inner join with Opportunties to bring the company that has opportunities
*/
CREATE Procedure [dbo].[sp_CompanyHasOpporunitiesView](
...
BEGIN
	...
	INNER join TBL_OPPORTUNITIES os on cp.COMPANIESID = os.COMPANYID
...
--exec [sp_CompanyHasOpporunitiesView] @orderBy='COMPANYNAME ASC' ,@pageSize=0,@pageNo=0,@franchiseeId=8,@searchText=''
GO
